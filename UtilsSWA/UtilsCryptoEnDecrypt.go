...
– Params:
  ...
  - raw_aad_suffix – additional not encrypted metadata suffix to include in the encrypted message, right after
    _RAW_AAD_PREFIX; null if not to be used

– Returns:
  - the encrypted message using the mentioned method, or null if the chosen algorithm was unable to process the data
    provided.
 */
...
– Params:
  ...
  - raw_password2 – the second character sequence to calculate the 2 keys from
  - raw_data – the data to encrypt
  - raw_aad_suffix – the associated authenticated data suffix used with the encrypted message; or null if not to be used

– Returns:
  - the original message text; null in case either the message was not encrypted using the parameters defined in the
...
– Returns:
  - 1st index AES key; 2nd index, MAC key; null if there's not enough memory for the keys calculation
 */
...
– Params:
  ...
  - associated_authed_data – additional not encrypted metadata to add to the tag, or null if not to use
